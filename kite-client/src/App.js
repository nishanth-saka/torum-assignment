...
let _scripCode = 1347585; //  RCOM-3375873, DLF-136414212
 function App() {
   ...
   useEffect(() => {
    ...
    //   await getHistoryData(obj);
    //  })

      getHistoryData(_scripCode);
   }, [tableColumns])

   function getSignal(close, rsi, sma, str, volume, rsiArray, volumeArray){
     let _return = null;
     ...
     if(close > sma && rsi >= 60 && _meanRSI <= 60 && volume > _meanVolume && str.Direction == 1){
      
      console.log('close', close);
      console.log('SUPER', str);
      console.log('rsi', rsi);
      console.log('_meanRSI', _meanRSI);
      console.log('volume', volume);
      console.log('_meanVolume', _meanVolume);
   ...
   function getTrendText(strObj){
    ...
    if(strObj && strObj.Up){
      ...
      _strText = `Trand: ${strObj.Direction == 1 ? 'Up' : 'Down'}`;
    }
    return _strText;
   }

   async function getHistoryData (scripCode){
     ...
     .then(async(response) => {
      ...
      if(response?.data){
        ...
        let _tableRowsArray = _.map(_responseArray, (obj, index) => {
          ...
          let _signal = getSignal(obj.close, obj.valueRSI, obj.valueSMA, obj.valueSTR, obj.volume, _rsiArray, _volumeArray);

          let _diffValue = (obj.index - _prevIndex);
          let _prevSign = '-';

          if((obj.valueSTR?.Up && _signal)){
            // console.log(`index: ${obj.index} - obj.STR: ${JSON.stringify(obj.valueSTR)}`);
            ...
            let _row = {
              ...
              vol: obj.volume,              
              sig: (_diffValue <= 1 && _prevSign !== 'BUY') ? '-' : 'BUY',
        ...
        if(_tableRowsArray[_tableRowsArray.length - 1]?.str === 'BUY'){
          console.log(`${_tableRowsArray.length} Rows for ${scripCode}`, _tableRowsArray[_tableRowsArray.length - 1]?.str);
          console.log('');
        } else {
          // console.log('.. ..');
        }
